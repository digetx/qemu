                             ...
                             uint64_t value, unsigned size)
{
    ...
    case 8: /* Control.  */
        ...
        /* Just change the period if the timer state wasn't changed.  */
        if ((old & 1) && ((old & 2) == (tb->control & 2))) {
            break;
        }
        /* The timer must not run if mode is one-shot and counter = 0
         * or mode is periodic and counter = load = 0.  */
        if (ptimer_get_count(tb->timer) == 0) {
            if (!(tb->control & 2)) {
                break;
            }
            if (tb->load == 0) {
            ...
            /* Avoid spurious interrupt trigger for the timer starting in
             * the periodic mode and counter = 0.  */
            if (tb->control & 2) {
                ptimer_set_count(tb->timer, tb->load);
            }
        }
